...
var cool = function(){console.log("Something Cool!")};
// -----------------------------------------------
...
The answer is [c] z and then y. x is never logged to the console, so its value is value is overriden twice. Even though, y is declared first in the code it is not declared for the program until the timeout function has finished. In that one second, the z has had more than enough time to be logged.
...
var callLater = function(timeout, callback){
  if(callback === undefined){
    //flipping the inputs since a single input would be taken as having set the timeout only
...
function max(a,b){
    ...
    if (a>b){
      ...
      return "the numbers are the equal";
    }
    else{
      return b;
    }
...
function rovarspraket(phrase){
    ...
    for(var i=0; i < phrase.length; i++){
      if(phrase[i] === "a" || phrase[i] === "e" || phrase[i] === "i" || phrase[i] === "o" || phrase[i] === "u" || phrase[i] ==" "){
        rovarString+= phrase[i];
      }
      else {
        rovarString+= phrase[i] + "o" + phrase[i];
      }
    }
    return rovarString;
}
